...
import {
  ...
  Instagram, Twitter, Linkedin, Youtube, MessageCircle, Plus,
  ArrowRight
} from "lucide-react";
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 border border-gray-700 shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className={`w-full bg-gray-800 text-white py-4 px-6 rounded-lg font-medium border border-gray-700 hover:bg-gray-700 transition-all duration-300 flex items-center justify-center gap-2 group mt-6 ${isDownloading ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                >
                  {isDownloading ? (
                    <>
...
export default function Contact({ settings }) {
  ...
  // Office locations (already sanitized by server action)
  ...
  return (
    <>
      ...
      <section className="py-20 bg-black">
        <div className="container mx-auto px-4 md:px-6 mt-20">
          ...
          >
            ...
            >
              ...
              {phoneNumbers.map((num, idx) => (
                <p key={idx} className="text-gray-400 mb-1">{num}</p>
              ))}
            </motion.a>
            ...
            >
              ...
              {emails.map((email, idx) => (
                <p key={idx} className="text-gray-400">{email}</p>
              ))}
            </motion.a>
            ...
            {locations.map((loc, idx) => (
              ...
              >
                ...
                <h3 className="text-2xl font-bold mb-2 text-white">{loc.title || 'VISIT US'}</h3>
                <p className="text-gray-400 text-sm leading-relaxed">{loc.address}</p>
                {locations.length > 1 && (
                  <button
                    onClick={() => {
                      setActiveLocationIdx(idx);
                      document.getElementById('location-map-section')?.scrollIntoView({ behavior: 'smooth' });
                    }}
          ...
          >
            ...
            >
              <h2 className="text-3xl md:text-4xl font-bold text-center mb-8 text-white tracking-tight">
              ...
              <div className="grid md:grid-cols-2 gap-12">
                ...
                <motion.div variants={fadeRight} className="space-y-4">
                  <motion.div whileHover={{ x: 10 }} className="flex items-center gap-3 text-gray-400 group cursor-pointer">
                    <Phone className="w-5 h-5 text-gray-500 group-hover:text-gray-300" />
                    <span className="group-hover:text-white transition-colors">{phoneNumbers.join(' · ')}</span>
                  </motion.div>
                  <motion.div whileHover={{ x: 10 }} className="flex items-center gap-3 text-gray-400 group cursor-pointer">
                    <Mail className="w-5 h-5 text-gray-500 group-hover:text-gray-300" />
                    <span className="group-hover:text-white transition-colors">{emails.join(', ')}</span>
                  </motion.div>
                  <motion.div whileHover={{ x: 10 }} className="flex items-center gap-3 text-gray-400 group cursor-pointer">
                    <Globe className="w-5 h-5 text-gray-500 group-hover:text-gray-300" />
                    <span className="group-hover:text-white transition-colors">www.threediamonds.ae</span>
                  </motion.div>
                  <motion.div whileHover={{ x: 10 }} className="flex items-center gap-3 text-gray-400 group cursor-pointer">
                    <MessageSquare className="w-5 h-5 text-gray-500 group-hover:text-gray-300" />
                    <span className="group-hover:text-white transition-colors">@goshi_warehouses_city</span>
                  </motion.div>
                </motion.div>
              </div>
            </motion.div>
      ...
      <section id="location-map-section" className="relative py-16 border-t border-gray-800 bg-gradient-to-b from-black to-gray-900">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold mb-4 bg-gradient-to-r from-gray-100 to-gray-400 bg-clip-text text-transparent italic tracking-tight">
              {activeLocation.title || 'Our Location'}
            </h2>
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3">
                {locations.map((loc, idx) => (
                  <button
                    ...
                    className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all border shadow-lg ${activeLocationIdx === idx
                        ? 'bg-white text-black border-white scale-105'
                        : 'bg-black/40 text-gray-500 border-gray-800 hover:border-gray-600 hover:text-white'
                      }`}
                  >
                    {loc.title || `Branch ${idx + 1}`}
                  </button>
                ))}
              </div>
            )}
          ...
          >
            ...
            <div className="relative w-full h-[400px] md:h-[500px] group">
              ...
              <motion.div
                ...
                className="absolute top-4 left-4 bg-black/90 backdrop-blur-md rounded-2xl p-5 border border-white/10 max-w-[280px] shadow-2xl"
              >
                ...
                <p className="text-xs text-gray-400 leading-relaxed mb-4">{activeLocation.address}</p>

                <div className="space-y-2.5 pt-3 border-t border-white/5">
                  <div className="flex items-center gap-3 text-xs">
                    <Phone className="w-3.5 h-3.5 text-gray-500" />
                    <span className="text-gray-300">{phoneNumbers[0]}</span>
                  </div>
                  <div className="flex items-center gap-3 text-xs">
                    <Mail className="w-3.5 h-3.5 text-gray-500" />
                    <span className="text-gray-300">{emails[0]}</span>
                  </div>
                </div>
                <button
                  onClick={openDirections}
                  className="w-full mt-4 bg-white/5 hover:bg-white text-white hover:text-black py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border border-white/10"
                >
                  Get Directions
                </button>
              </motion.div>
            ...
            {/* Map Footer */}
            <div className="p-4 bg-black/40 border-t border-white/5 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <div className="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse"></div>
                  <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Active Interactive Map</span>
                </div>
                {activeLocation.lat && (
              ...
              <div className="flex gap-4">
                <motion.a
                  ...
                  rel="noopener noreferrer"
                  whileHover={{ x: 3 }}
                  className="text-[10px] text-gray-500 hover:text-white transition-colors flex items-center gap-1.5 font-bold uppercase tracking-wider"
                >
