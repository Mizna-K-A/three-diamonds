...
/**
 * Gets admin notification emails from SiteSettings, falling back to .env ADMIN_EMAIL.
 * Returns null if no emails are configured anywhere.
 */
export async function getAdminEmails() {
  try {
    await connectDB();
    const settings = await SiteSettings.findOne().select('adminEmails');
    if (settings?.adminEmails?.length > 0) {
      return settings.adminEmails;
    }
  } catch (err) {
    console.error('Error fetching admin emails from DB:', err);
  }
 ...
 */
export async function sendMail({ to, subject, text, html, attachment }) {
